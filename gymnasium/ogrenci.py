...
def ogrenciEdit(db, form, id):
    try:
        file = form.ogrenciFoto.data
        if file:
            fotoname = secure_filename(file.filename)
            upload_path = os.path.join('uploads', session['dbAdi'], 'foto')
            ogrenciFoto = save_file(file, upload_path, fotoname)
        else:
            ogrenciFoto = None
        ogrenciTC = form.ogrenciTC.data
        ogrenciAd = form.ogrenciAd.data
        ...
        veli2Telefon = form.veli2Telefon.data
        toplamOdeme = form.toplamOdeme.data
        values = (ogrenciFoto, ogrenciTC, ogrenciAd, ogrenciSoyad, ogrenciTelefon, ogrenciYas, ogrenciDogumTarihi, ogrenciBrans, ogrenciGrup, veliAd, veliTelefon, veli2Ad, veli2Telefon, toplamOdeme)
